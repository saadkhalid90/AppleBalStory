function drawAppFlow(){
    ...
    const svg = d3.select('svg.appleFlow');
    ...
    async function readAndDraw(){
      let PakGeo = await d3.json('GeoData/PakCons.topojson');
      let AppleVCGeo = await d3.json('GeoData/AppleVCDists.topojson');
      let CommAgLocs = (await d3.csv('Data/CommAgLocs.csv')).filter(d => d.Province != 'None');
            ...
            if (over){
              d3.select('div.appleFlowContain').append('div')
                              ...
                              .html(
                                ...
                                `
                                  <p>
                                    <span class='varName'>Share</span>: <span>${round2Dec(datum.Perc, 1)}%</span>
                                  </p>
                                `
                              )
                              .styles({
